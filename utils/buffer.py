...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        ...
        for odim, adim in zip(obs_dims, ac_dims):
            ...
            self.ac_prob_buffs.append(np.zeros((max_steps, adim)))
    ...
    def push(self, observations, actions, actions_prob, rewards, next_observations, dones):
        ...
        if self.curr_i + nentries > self.max_steps:
            ...
            for agent_i in range(self.num_agents):
                ...
                self.ac_buffs[agent_i] = np.roll(self.ac_buffs[agent_i],
                                                 rollover, axis=0)
                self.ac_prob_buffs[agent_i] = np.roll(self.ac_prob_buffs[agent_i],
                                                 rollover, axis=0)
        ...
        for agent_i in range(self.num_agents):
            ...
            self.ac_prob_buffs[agent_i][self.curr_i:self.curr_i + nentries] = actions_prob[agent_i]
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        ...
        return ([cast(self.obs_buffs[i][inds]) for i in range(self.num_agents)],
                ...
                [cast(self.ac_prob_buffs[i][inds]) for i in range(self.num_agents)],
                ret_rews,
